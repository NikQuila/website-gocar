...
interface VehicleCatalogRetroProps {
  ...
  showFilters?: boolean;
  // Global filter color control
  filterGlobalColor?: string;
  // Background effects
  backgroundType?: 'solid' | 'gradient' | 'gradient-radial';
...
export const VehicleCatalogRetro = ({
  ...
  // Global filter color control
  filterGlobalColor,
  // Background effects with defaults
  backgroundType = 'gradient',
  gradientStartColor = '#1a1a1a',
  gradientEndColor = '#2d2d2d',
  backgroundOpacity = 1,
  backgroundBrightness = 1,
  // Filter button colors with defaults
  ...
  filterButtonColors = [
    {
      ...
      activeButtonBgColor: '#4a5568',
      activeButtonTextColor: '#ffffff',
      activeButtonBorderColor: '#ffffff',
      containerBgColor: '#2a2a2a',
  ...
  // Aplicar color global a las etiquetas de sliders si está definido
  ...
  const getBackgroundStyle = () => {
    ...
    switch (backgroundType) {
      case 'gradient':
        background = `linear-gradient(135deg, ${gradientStartColor}, ${gradientEndColor})`;
        break;
  ...
  const applyRetroFilters = () => {
    ...
    if (selectedRetroTransmissions.length > 0) {
      filtered = filtered.filter((vehicle) => {
        ...
        const normalizedTransmission = normalizeTransmission(
          vehicle.transmission
        );
        return selectedRetroTransmissions.includes(normalizedTransmission);
      });
    }
    ...
    const fetchVehicles = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          // Normalizar transmisiones para eliminar duplicados
          const transmissions = Array.from(
            new Set(
              vehiclesData
                .map((v) => v.transmission)
                .filter((t) => t !== undefined && t !== null)
                .map((t) => normalizeTransmission(t))
            )
          ).sort();
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <div>
          {/* Filters Section with Full Width Dark Background */}
          <div
            style={{
              ...getBackgroundStyle(),
              color: textColor,
            ...
            <div className='max-w-7xl mx-auto px-4 sm:px-6'>
              {showFilters && (
                <VehicleCatalogRetroFilters
                  ...
                  // Global filter color
                  filterGlobalColor={filterGlobalColor}
                  // Button colors
                  filterButtonBgColor={filterButtonBgColor}
                  filterButtonTextColor={filterButtonTextColor}
                  filterButtonBorderColor={filterButtonBorderColor}
                  resetButtonBgColor={resetButtonBgColor}
                  resetButtonTextColor={resetButtonTextColor}
                  resetButtonBorderColor={resetButtonBorderColor}
                  // Slider colors (aplicando color global si está definido)
                  sliderLabelBgColor={sliderLabelBgColor}
                  sliderLabelTextColor={effectiveSliderLabelTextColor}
                  sliderLabelBorderColor={sliderLabelBorderColor}
                  // Sizes
                  titleSize={titleSize}
              ...
              {/* Results Count - Inside Dark Background */}
              {filteredVehicles.length > 0 && (
                <div className='text-center mt-4'>
                  <p
                    className='text-sm'
                    style={{ color: effectiveSliderLabelTextColor }}
                  >
                    <span
          ...
          {/* Results Section with Normal Background */}
          <div className='max-w-7xl mx-auto px-4 sm:px-6'>
