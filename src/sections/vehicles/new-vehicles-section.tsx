...
const NewVehiclesSection = () => {
  ...
  // Efecto para detectar el scroll y mostrar el botón
  useEffect(() => {
    // Solo aplicar el efecto de scroll en dispositivos móviles
    if (isMobile) {
      // Inicializar el estado como falso para móviles
      ...
      // En tablet y desktop, siempre mostrar el botón
      setShowWhatsAppButton(true);
    }
  }, [isMobile]);
  ...
  return (
    <div className='min-h-screen bg-gray-50 dark:bg-dark-bg'>
      {/* Botón de WhatsApp flotante (para todos los tamaños) */}
      <div
        className={`fixed z-50 transition-all duration-300 ${
          isMobile
            ? 'bottom-6 right-6'
            : isTablet
            ? 'top-[calc(var(--navbar-height)+80px)] right-6'
            : 'top-[calc(var(--navbar-height)+120px)] right-10'
        } ${
          isMobile
            ? showWhatsAppButton
      ...
      >
        <Link href={whatsappUrl} target='_blank' rel='noopener noreferrer'>
          <Button
            isIconOnly
            className='bg-[#25D366] text-white rounded-full hover:bg-[#22c35e] hover:shadow-lg transition-all duration-200 shadow-md w-12 h-12 sm:w-14 sm:h-14'
            aria-label='WhatsApp'
          >
            <Icon icon='logos:whatsapp-icon' className='text-xl sm:text-2xl' />
          </Button>
        </Link>
      </div>
